...
def CalculateForegrounds(Bg, img, kernel, pts, ChangeBgmFlag, AreaThreshold, GrayThreshold, signalLamp, KeyPointsFlag):
    ...
    for c in contours2:
        ...
        if perimeter > AreaThreshold:
            ...
            if KeyPointsFlag == 0:
                cv2.drawContours(img, c, -1, (0, 0, 255), 1)  # 画出轮廓（红色）
...
class CamShow(QMainWindow,Ui_MainWindow):
    ...
    def mousePressEvent(self, event):
        ...
        if event.buttons() == QtCore.Qt.LeftButton and self.start_pos == None and self.resetLabel == 2:
            ...
            self.start_x = int((self.start_pos.x() - 305) * self.init_W / 1280)
            self.start_y = int((self.start_pos.y() - 60) * self.init_H / 720)
            self.resetLabel = 1
            # print(self.start_x, self.start_y)
        elif event.buttons() == QtCore.Qt.LeftButton and self.start_pos != None and self.end_pos == None and self.resetLabel == 1:
            self.end_pos = event.pos()
            # self.mousePress = True
            self.end_x = int((self.end_pos.x() - 305) * self.init_W / 1280)
            self.end_y = int((self.end_pos.y() - 60) * self.init_H / 720)
            self.resetLabel = 0
        ...
        # 矩形区域对角两点获取
            ...
            # self.mousePress = True
            self.rectangle_start_x = int((self.rectangle_start_pos.x() - 305) * self.init_W / 1280)
            self.rectangle_start_y = int((self.rectangle_start_pos.y() - 60) * self.init_H / 720)
            # self.resetLabel = 1
            ...
            # self.mousePress = True
            self.rectangle_end_x = int((self.rectangle_end_pos.x() - 305) * self.init_W / 1280)
            self.rectangle_end_y = int((self.rectangle_end_pos.y() - 60) * self.init_H / 720)
            # self.resetLabel = 0
        ...
        # 圆形区域直径两点获取
            ...
            # self.mousePress = True
            self.circle_start_x = int((self.circle_start_pos.x() - 305) * self.init_W / 1280)
            self.circle_start_y = int((self.circle_start_pos.y() - 60) * self.init_H / 720)
            # self.resetLabel = 1
            ...
            # self.mousePress = True
            self.circle_end_x = int((self.circle_end_pos.x() - 305) * self.init_W / 1280)
            self.circle_end_y = int((self.circle_end_pos.y() - 60) * self.init_H / 720)
            # self.resetLabel = 0
    ...
    def PrepCamera(self):
        try:
            ...
            self.camera.set(cv2.CAP_PROP_FRAME_WIDTH, self.init_W)
            self.camera.set(cv2.CAP_PROP_FRAME_HEIGHT, self.init_H)
    ...
    def TimerOutFun(self):
        ...
        if self.DetectFlag:
            ...
            if success:
                ...
                cv2.putText(img, TimeText, (970, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1,
                            cv2.LINE_AA)
                ...
                if self.KeyPointsFlag == 1:
                    ...
                    #                                                      self.keyPoints_all)
                    self.keyPoints, self.Angle, self.keyPoints_all = PoseDetection(self.Image, self.init_W, self.init_H,
                                                                                   self.device, self.net, 10,
                                                                                   self.keyPoints, self.keyPoints_all)
                        # print('nodeted')
                    # time_end = time.time()
                ...
                if rect==0:
                    ...
                    ########################视频上绘制时间##############################
                    # TimeText = 'Time:' + datetime.datetime.now().strftime('%Y%m%d-%H:%M:%S.%f')
                    # cv2.putText(self.Image, TimeText, (970, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1,
                    #                   cv2.LINE_AA)
                    ########################视频上绘制时间##############################
                    ...
                    cv2.putText(self.Image, CenterText, (1100, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1,
                                      cv2.LINE_AA)
                    ########################视频上绘制信号灯和中心点坐标##############################
                    ...
                    if self.KeyPointsFlag == 1:
                        ...
                        cv2.putText(self.Image, AngleText, (1000, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1,
                                    cv2.LINE_AA)
                    ...
                    if len(self.pts) <= 10:
                        self.SpeedAll = [0]
                    elif len(self.pts) > 100:
                        self.pts = self.pts[-100:]  # 释放部分内存，避免中心点序列一直积累占用空间
                    self.speed = self.SpeedAll[-1]

                    SpeedText = '  Speed: [' + str(round(self.speed, 2)) + ']'
                    cv2.putText(self.Image, SpeedText, (1100, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1,
                                cv2.LINE_AA)
            ...
            if success:
                ...
                TimeText = datetime.datetime.now().strftime('%Y%m%d-%H:%M:%S.%f')
                cv2.putText(img, TimeText, (970, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1,
                            cv2.LINE_AA)
                self.Image = img
                # self.Image = img  # self.ColorAdjust(img)
                ...
                self.Image_num += 1
                if self.RecordFlag:
                    self.video_writer_original.write(img)
                    with open(self.LogCSVPath, "a", newline='') as datacsv:
                        # dialect为打开csv文件的方式，默认是excel，delimiter="\t"参数指写入的时候的分隔符
                        csvwriter_ori = csv.writer(datacsv, dialect=("excel"))
                        csvwriter_ori.writerow([TimeText])
    ...
    def RecordCamera(self):
        ...
        if tag == '保存':
            ...
            if self.DetectFlag:
                ...
                self.LogCSVPath = self.LogPath + 'Logs_noDetect_' + time.strftime('%Y%m%d%H%M%S',
                                                                         time.localtime(time.time())) + '.csv'
                # 写记录文档表头
                with open(self.LogCSVPath, "a", newline='') as datacsv:
                    csvwriter_ori = csv.writer(datacsv, dialect=("excel"))
                    csvwriter_ori.writerow(["日期(年月日)-时间(时：分：秒：毫秒)"])
